...
/** Green smooth wave chart for HRV on Recovery Detail. */
public class HrvChartView extends View {
    ...
    private void init() {
        line.setStyle(Paint.Style.STROKE); line.setStrokeWidth(3f); line.setColor(0xFF4CAF50);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        fill.setShader(new LinearGradient(0,0,0,h,0x554CAF50,0x004CAF50,Shader.TileMode.CLAMP));
        ...
        for (int i=0; i<data.length; i++) {
            float x=i*sx, y=h-((data[i]-min)/rng)*h*0.8f-h*0.1f;
            if (i==0) { fp.moveTo(x,h); fp.lineTo(x,y); lp.moveTo(x,y); }
            else {
                float px=(i-1)*sx, py=h-((data[i-1]-min)/rng)*h*0.8f-h*0.1f, cpX=(px+x)/2f;
